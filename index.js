...
const GITHUB_TOKEN = process.env.GITHUB_TOKEN; // Token Akun Utama (icatpojan)
const BOT_GITHUB_TOKEN = process.env.BOT_GITHUB_TOKEN || process.env.GITHUB_TOKEN; // Token Akun Kedua (bot)

const REPO_OWNER = process.env.REPO_OWNER || "icatpojan";
const REPO_NAME = process.env.REPO_NAME || "project-portofolio";

const octokitMain = GITHUB_TOKEN ? new Octokit({ auth: GITHUB_TOKEN }) : null;
const octokitBot = BOT_GITHUB_TOKEN ? new Octokit({ auth: BOT_GITHUB_TOKEN }) : octokitMain;
...
// 2. ISSUE AUTOMATION (Akun Utama / Bot)
async function handleIssueActivity(timestamp) {
    ...
    // Create Issue via Bot Token (agar diselesaikan/direview akun utama) atau Main Token
    const { data: issue } = await octokitBot.rest.issues.create({
        owner: REPO_OWNER,
        repo: REPO_NAME,
        title: issueTitle,
        body: template.body,
    ...
    // Add Comment & Close via Main Token
    await octokitMain.rest.issues.createComment({
        owner: REPO_OWNER,
        repo: REPO_NAME,
        issue_number: issue.number,
        body: `Automated review check at ${timestamp}: Root cause identified. Resolving issue.`
    });
    console.log(`[ISSUE COMMENTED] #${issue.number} by Main Account`);

    await octokitMain.rest.issues.update({
        owner: REPO_OWNER,
        repo: REPO_NAME,
        issue_number: issue.number,
        state: "closed"
...
// 3. PULL REQUEST AUTOMATION (Dibuat oleh BOT_GITHUB_TOKEN)
async function handlePullRequestActivity(timestamp) {
    if (!octokitBot) {
        console.log(`[PR SKIPPED] GITHUB_TOKEN / BOT_GITHUB_TOKEN belum diisi di .env`);
        return handleDirectCommit(timestamp);
    }
    ...
    // Open Pull Request via BOT Account
    const { data: pr } = await octokitBot.rest.pulls.create({
        owner: REPO_OWNER,
        repo: REPO_NAME,
        title: `feat: add utility snippet (${timeId.split("T")[1]})`,
        head: branchName,
        base: "main",
        body: `Automated PR generated by bot account at ${timestamp}.\n\nReady for Code Review.`
    });
    console.log(`[PR CREATED BY BOT] #${pr.number}: ${pr.title} (Branch: ${branchName})`);
    return pr;
}

// 4. CODE REVIEW & MERGE AUTOMATION (Direview & Merged oleh GITHUB_TOKEN Akun Utama!)
async function handleCodeReviewActivity(timestamp) {
    ...
    // List open PRs
    const { data: openPRs } = await octokitMain.rest.pulls.list({
        owner: REPO_OWNER,
        repo: REPO_NAME,
        state: "open"
    });
    ...
    let pr;
    if (openPRs.length === 0) {
        console.log(`[REVIEW] Tidak ada PR yang terbuka. Membuka PR baru via Bot terlebih dahulu...`);
        pr = await handlePullRequestActivity(timestamp);
    } else {
        pr = openPRs[0];
    }
    ...
    // Submit Code Review oleh Akun Utama (icatpojan)
    await octokitMain.rest.pulls.createReview({
        owner: REPO_OWNER,
        repo: REPO_NAME,
        pull_number: pr.number,
        event: "APPROVE",
        body: `LGTM! Code review completed at ${timestamp}. Verified changes and approving PR.`
    });
    console.log(`[CODE REVIEWED BY MAIN ACCOUNT] Approved PR #${pr.number}`);

    // Merge PR oleh Akun Utama
    await octokitMain.rest.pulls.merge({
        owner: REPO_OWNER,
        repo: REPO_NAME,
        pull_number: pr.number,
        merge_method: "squash"
    });
    console.log(`[PR MERGED BY MAIN ACCOUNT] Merged PR #${pr.number}`);
